...
struct STLazySum {
    int n;
    vector<int> tr, lz;
    STLazySum(int n) : n(n), tr(4*n + 8), lz(4*n + 8) {}
...
void solve() {
    ...
    for (int i = 0 ; i < n ; i++) {
        cin >> a[i];
        fs += a[i];
        segtree.update(i , i + 1 , fs);
        sum.update(i , i + 1 , a[i]);
    }
    while (q--) {
        int stat; cin >> stat; 
        if (stat == 1) {
            int k, u; cin >> k >> u;
            segtree.update(k - 1 , n , -a[k - 1]); 
            segtree.update(k - 1 , n , u);
            sum.update(k - 1 , k , u);
            a[k - 1] = u;
        }
        else {
            int a , b; cin >> a >> b;
            cout << max(0ll , segtree.query(a - 1 , b) - sum.query(0 , a - 1)) << '\n';
        }
    }
}
