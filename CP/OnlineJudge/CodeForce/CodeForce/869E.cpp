...
void solve() {
    ...
    for (int i = 0 ; i < q ; i++) {
        int type; cin >> type; 
            ...
            g[i].emplace_back(x2);
            g[i].emplace_back(y2);
            if (mp.find({x1 - 1 , y1 - 1}) == mp.end()) bit.fakeUpdate(x1 - 1 , y1 - 1) , mp[{x1 - 1 , y1 - 1}] = true;
            if (mp.find({x2 , y2}) == mp.end()) bit.fakeUpdate(x2 , y2) , mp[{x2 , y2}] = true;
            if (mp.find({x1 - 1 , y2}) == mp.end()) bit.fakeUpdate(x1 - 1 , y2) , mp[{x1 - 1 , y2}] = true;
            if (mp.find({x2 , y1 - 1}) == mp.end()) bit.fakeUpdate(x2 , y1 - 1) , mp[{x2 , y1 - 1}] = true;
    }
    bit.init();
    vector<int> pw(q + 1);
    pw[0] = 1ll;
    ...
    for (int i = 0 ; i < q ; i++) {
        ...
        if (type == 1) {
            ...
            int x2 = g[i][3];
            int y2 = g[i][4]; 
            bit.update(x1 - 1, y1 - 1 , pw[j]);
            bit.update(x2 , y2 , pw[j]);
            bit.update(x1 - 1 , y2 , -pw[j] + mod); 
            bit.update(x2 , y1 - 1 , -pw[j] + mod);
            mm[{x1 , y1 , x2 , y2}] = i;
        }
        if (type == 2) {  
            int x1 = g[i][1];
            ...
            int x2 = g[i][3];
            int y2 = g[i][4]; 
            int k = mm[{x1 , y1 , x2 , y2}];
            //cerr << k << '\n';
            mm[{x1 , y1 , x2 , y2}] = -1;
            bit.update(x1 - 1 , y1 - 1, -pw[k + 1] + mod);
            bit.update(x2 , y2 , -pw[k + 1] + mod);
            bit.update(x1 -1 , y2 , pw[k + 1]); 
            bit.update(x2 , y1 -1 , pw[k + 1]);
        }
        if (type == 3) {
            int x1 = g[i][1];
            int y1 = g[i][2]; 
